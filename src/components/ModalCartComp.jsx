...
export default function ModalCartComp({ item, handleClose, openModal, handleBtnKeranjang }) {
  ...
  function updateQty(type){
    if(type == "+"){
      //prev : parameter setter state untuk mengamnil nilaiset sebelumnya
      setQty((prev) => prev+1);
    } else {
      // Jika -, cmn boleh diupdate max sampe 1 pengurangan
      if(qty > 1){
        setQty((prev) => prev-1);
      }
    }
      ...
      <Modal dismissible show={openModal} onClose={handleClose}>
        ...
        <ModalBody>
          ...
          <div className="flex justify-end mt-4">
            <ButtonGroup>
              <Button color="blue" onClick={() => updateQty("-")}><FaMinus className="w-4 h-4"/></Button>
              <Button color="alternative">{qty}</Button>
              <Button color="blue" onClick={() => updateQty("+")}><FaPlus className="w-4 h-4"/></Button>
            </ButtonGroup>
          </div>
        </ModalBody>
